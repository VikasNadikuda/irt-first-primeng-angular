...
export class OrgManageStateComponent implements OnInit {
  @BlockUI('viewState1') loader: NgBlockUI;
  @ViewChild('fileInputO1') fileInput: FileUpload;
  edit=false
  title="Add State"
  ...
  ngOnInit(): void {
    ...
    this.getCountry()
    this.libraryService.getStates(this.orgID).subscribe(
      (success)=>{
        this.loader.stop()
        console.log(success)
  ...
  upload(){
    ...
    this.libraryService.uploadFile(formdata,userID,'STATE',this.orgID).subscribe(
      (success)=>{
        console.log(success)
        this.spinner.hide()
  ...
  save(type){
    ...
    if(this.edit){
      ...
      const body={
        ...
        "Lib_Name": "STATE",
        "Lib_value":this.addState.controls.state.value,
        "Parent_Key":this.addState.controls.country.value,
        "UID":uid,
        "ORGID":this.orgID,
        "STATUS":this.addState.controls.STATUS.value,
      }
