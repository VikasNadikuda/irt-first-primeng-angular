...
export class OrgManageControllerVocabularyComponent implements OnInit {
  @BlockUI('viewLibrary1') loader: NgBlockUI;
  @ViewChild('fileInputO3') fileInput: FileUpload;
  edit=false
  title="Add Library"
...
status=[
  ...
  ngOnInit(): void {
    ...
    this.libraryService.getLibraries(this.orgID).subscribe(
      (success)=>{
        this.loader.stop()
...
upload(){
  ...
  this.libraryService.uploadFile(formdata,userID,'OTHER',this.orgID).subscribe(
    (success)=>{
      console.log(success)
      this.spinner.hide()
