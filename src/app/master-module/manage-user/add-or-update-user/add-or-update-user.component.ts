...
export class AddOrUpdateUserComponent implements OnInit {
  ...
  getRoles(){
    this.orgService.getMasterRoles().subscribe(
      (success)=>{
        if(success!=undefined && success?.length!=0){
  ...
  saveOrganisation(){
    ...
    this.orgNames.forEach((org)=>{
      console.log(org)
      if(org.ORGANIZATION_NAME==userObj.ORGANIZATIONNAME){
        userObj.ORGANIZATIONID=org.ID;
      }
    })
    console.log(userObj)
    if(this.submitButton=='ADD'){
      ...
      userObj.ORGANIZATIONNAME=this.editUser.Organization
