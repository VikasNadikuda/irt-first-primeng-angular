...
export class ManageCityComponent implements OnInit {
  @BlockUI('viewCity') loader: NgBlockUI;
  @ViewChild('fileInput2') fileInput: FileUpload;
  edit=false
  title="Add City"
  ...
  ngOnInit(): void {
    ...
    this.libraryService.getCities().subscribe(
      (success)=>{
        this.loader.stop()
  ...
  upload(){
    ...
    this.libraryService.uploadFile(formdata,userID,'CITY').subscribe(
      (success)=>{
        console.log(success)
        this.spinner.hide()
