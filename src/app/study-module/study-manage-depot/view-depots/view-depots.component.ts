...
export class ViewDepotsComponent implements OnInit {
  @BlockUI('StudyviewDepot') loader: NgBlockUI;
  statuses: any[];
  ...
  editDepot(data){
    console.log(data)
    this.router.navigate(['study/manageDepot/addOrEditDepot'],{state:{depotDetails:data}})
  }
  exportPdf() {
    const doc = new jsPDF();
    const rows=[]
  ...
  editDepotStatus(){
    this.router.navigate(['study/manageDepot/depotStatus'],{state:{depotDetails:this.depots}})
  }
