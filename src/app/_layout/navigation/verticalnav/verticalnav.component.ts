...
import { MenuSettingsConfig } from '../../settings/menu-settings.config';
@Component({
...
export class VerticalnavComponent implements OnInit {
  ...
  constructor(
    ...
    // private _organisationMenu:organisationMenu,
  ...
  ngOnInit() {
    ...
    this._menuSettingsService.config
      ...
      console.log(this._menuSettingsConfig)
    // TODO Patch to reset menu after login
  }
  ngDoCheck(){
    // console.log(this.menu._defaultConfig)
    this._menuSettingsConfig=this.menu._defaultConfig
    // this.resetMainMenu()
    this.setActiveRouteInNavbar()
  }
  ...
  setActiveRouteInNavbar () {
    for (let i = 0; i < this._menuSettingsConfig.vertical_menu.items.length; i++) {
        ...
        for (let j = 0; j < this._menuSettingsConfig.vertical_menu.items[i].submenu.items.length; j++) {
          if (!this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].submenu &&
            ...
            for (let k = 0; k < this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].submenu.items.length; k++) {
              ...
              else if(this.router.url.includes(this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].submenu.items[k].page)) {
                this._menuSettingsConfig.vertical_menu.items[i]['isSelected'] = true;
                this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j]['isSelected'] = true;
                this._menuSettingsConfig.vertical_menu.items[i].isOpen = true;
            ...
            if(this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].deepChild){
              var childs=this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].deepChild
              if(childs.indexOf(this.router.url)>-1){
                // console.log(childs,this.router.url)
                this._menuSettingsConfig.vertical_menu.items[i]['isSelected'] = true;
                this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j]['isSelected'] = true;
                this._menuSettingsConfig.vertical_menu.items[i].isOpen = true;
              }
      ...
      //     for (let j = 0; j < this._menuSettingsConfig.vertical_menu.items[i].submenu.items.length; j++) {
      //       for (let k = 0; k < this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].submenu.items.length; k++) {
      //         if (this.router.url.includes(this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j].submenu.items[k].page)) {
      //           this._menuSettingsConfig.vertical_menu.items[i]['isSelected'] = true;
      //           this._menuSettingsConfig.vertical_menu.items[i].submenu.items[j]['isSelected'] = true;
      //           this._menuSettingsConfig.vertical_menu.items[i].isOpen = true;
  ...
  callFunction(event, child, isSubmenuOfSubmenu) {
    ...
    console.log(child,isSubmenuOfSubmenu)
    if (this[methodName]) {
